...
from bot_conf import get_setting
...
def check_message(response):
    ...
    if 'text' in response[0]['message']:
        ...
        # Условие выполняется только в том случае, если внутри ответа на GET-запрос [message][text] есть текст /help.
        if '/help' in response[0]['message']['text']:
            # Ф-ция для записи сообщений в log-файл.
            log('В [message][text] найдено совпадение по ключу: "/help"')

            # Отправляем в ответ help-лист.
            # Ф-ция для записи сообщений в log-файл
            log('"Отправляем в ответ help-лист"')
            # Текст ответа.
            text = f"Бот - прислужник. \n\n*Помогает получать расписание занятий с сайта www.avalon.ru \n\n*Рассылает расписание в день занятий в личные сообщения. \n\n[Работа]\n/start - Начать работу с ботом. \n/help - Помощь по командам. \n\n[Расписание] \n/day - Расписание на сегодня (при отстутсвии занятий, бот пришлет занятия в ближайший день. \n\n[Остальное] \nТехническая поддержка - @vladKorolev"
            # Метод запроса.
            req_url = "sendMessage"
            # Параметры запроса.
